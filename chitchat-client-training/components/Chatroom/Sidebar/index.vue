...
<template>
  <div>
    <div class="relative flex flex-col h-full w-full bg-white transition-all duration-200 ease-out">
      ...
      <article
        id="scrollbar"
        class="flex flex-col overflow-y-scroll scroll-smooth w-full h-[414px] transition-all duration-200 ease-out"
      >
          ...
          <template v-if="chatroom_list?.length > 0">
            ...
            >
              <div id="pin" class="absolute top-0 right-9 transition-all duration-200 ease-linear">
                <Icon
                  name="eos-icons:push-pin-outlined"
                  class="text-text"
                  :class="chatroom_show?._id == item._id ? 'pin_show' : ''"
                />
              </div>
      ...
      <div v-if="is_show.new_menu" class="z-50 absolute bottom-[68px] right-10 w-fit h-fit">
        <ChatroomSidebarMenu />
      </div>
